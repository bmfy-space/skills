...
<HARD-GATE>
如果当前运行环境是 Claude Code，执行本技能前不要自动切换到 Claude Code 的内置 plan mode；本技能自身负责通过反问、落盘 `plan.md` / `todo.md` 来完成规划流程。
...
- **先计划后编码**：计划未落盘之前，不进入实现；计划落盘并向用户展示摘要后，除非仍有阻塞或未确认的实现边界，否则无需等待用户批准，直接开始执行。
...
6. 在该文件夹中创建 `todo.md`
7. 在对话中简要展示计划摘要
8. 直接开始按 `todo.md` 执行；只有在仍存在阻塞、破坏性操作或未确认的实现边界时，才需要再次询问用户

计划落盘并展示摘要后，不需要等待用户批准即可实现。
...
> 我已把计划和待办写入 `<plan-path>` 与 `<todo-path>`。摘要如下：<简短摘要>。接下来我会直接按待办开始执行；如果遇到阻塞或需要确认边界，会再向你确认。
